    ...
    <form action="" name="form" method="post" class="form form-horizontal"
        ...
        <div class="cl pd-5  mt-10">
				<span class="l" style="margin-left:5px;">
				 <a class="btn btn-primary radius" onclick="open_layer('添加','adminmarketadd','900','450')" href="javascript:;">
    ...
    <div style="width: 98%; margin-left: 10px">
        <table id="example" class="table table-striped table-bordered">
            ...
            <tr>
                ...
                <th style="text-align: center;width: 5%">区id</th>
                <th style="text-align: center;width: 10%">市场名称</th>
                <th style="text-align: center;width: 10%">市场图片</th>
                <th style="text-align: center;width: 10%">省</th>
                <th style="text-align: center;width: 10%">市</th>
                <th style="text-align: center;width: 10%">区</th>
    ...
    $(document).ready(function() {
        ...
        $('#example').DataTable( {
            ...
            "columns": [
                ...
                { "data": "market_name" },
                { "data": function(obj){
                        return "<span><center><img style='max-width:80px' src='../upload/"+obj.market_img+"' onclick=\"open_layer('查看','../upload/"+obj.market_img+"','600','450')\"/></center></span>"
                    }
                },
                { "data": "market_province" },
                { "data": "market_city" },
                { "data": "market_region" },
                { "data": "market_address" },
                { "data":  function(obj){
                        if(obj.market_status==0){
                            return "<span><center>未启用</center></span>"
                        }else if(obj.market_status==1){
                            return "<span><center>启用中</center></span>"
                        }
                    }
                },
                {
                ...
                { "data": function(obj){
                        if(obj.market_status==0){
                            return "<a onclick=\"open_layer('编辑','adminmarketedit?id="+obj.id+"','900','450')\">编辑</a>&nbsp;" +
                                   "<a onclick=\"startobj("+obj.id+")\">启用</a>&nbsp;<a onclick=\"deleteobj(" + obj.id + ")\">删除</a></center></span>"
                        }else if(obj.market_status==1){
                            return  "<a onclick=\"open_layer('编辑','adminmarketedit?id="+obj.id+"','900','450')\">编辑</a>&nbsp;" +
                                    "<a onclick=\"endobj("+obj.id+")\">停用</a>&nbsp;<a onclick=\"deleteobj(" + obj.id + ")\">删除</a></center></span>"
                        }
                    }
                }
            ],
